...
In transcriptomic data analysis, EDA includes exploring the read count distribution, detection of lowly-expressed genes and poor-quality samples/cells, determine biological and technical sources of gene expression variation, and detect confounding factors.

In this blog post, you will learn how to perform EDA by exploring an example gene expression data set, hoping to provide a methodological but flexible framework for you to guide your own data analysis.
...
-   Learn of helpful visualization tools to explore your data.
...
In any EDA, we first need to understand what the sample variables in the metadata stand for in order to interpret any insights derived from them accurately. A second important consideration is that, even when there are well-established QC metrics used to filter samples, exploring a broader set of them is conveninent to find unexpected sample quality differences.
...
-   [SAMtools](http://www.htslib.org) [@danecek2021] statistics for chromosome-specific mapping reads. Of special interest are the reads mapping to the mitochondrial and sex chromosomes (see explanation in Step 1 XXXXX):
...
For more details of the computation of these metrics please refer to *recount3* [documentation](http://rna.recount.bio/docs/quality-check-fields.html) and each tool's manual.

# 1. Exploring count distribution
...
Explore sex differenes: sexual chr % can be helpful in, for instance, confirming the sex of the individual sequenced based on alignments to sex chromosomes
...
Very unshared is my personal excitement when performing this step. But EDA is extremely illuminating when performed comprehensively and you may find an aswer to your data issues or lack of signals by looking back to this step.
