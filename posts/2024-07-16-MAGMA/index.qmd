...
After running the gene analysis, either by PC regression or SNP-wise analysis, the resulting gene *p*-values are subsequently transformed to z-scores ($z_g$) to be introduced in the gene-set analysis. These z-scores capture the association of the genes with the phenotype.

## Gene-set analysis

Before entering into what is a gene-set analysis and what inforamation it can provide, we must clarify that the gene-set analysis operates in the same way using gene analysis results based on the PC linear regression model or the SNP-wise analysis. All that's necessary to run the gene-set analysis are the gene *p*-values obtained from the gene analysis.
