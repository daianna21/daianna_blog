...
::: {style="background-color:#F2F2F2; padding: 15px"}
✔️ Both self-contained and competitive gene-set analyses are implemented through more general gene-level regression models that can accommodate combinations of gene sets and gene properties such as gene expression levels and gene size to test for their effects and correct for potential confounding effects. This can be useful, for example, to test if variables such as differential expression between tissues (🧠 vs 🩸) or conditions (⚕️ vs 💊) have an effect on the phenotype associations.

Moreover, analyzing gene sets evidently reduces the number of output tests, but note that one test per gene still has to be performed in the the gene analysis.
:::
...
MAGMA is a powerful computational tool for extensible and flexible gene and gene-set analyses.
...
Contents of this post are based on the original MAGMA publication and Supplementary File 1, and the official documentation website linked above.
