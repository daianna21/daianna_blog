...
![](images/sampling.png){fig-align="center" width="2525"}
...
Now, as you may suspect, the decrease in counts for genes in condition [**A**]{style="color: #FF83FA"} after sampling results in false discoveries of negative differential expression.
...
Library size normalization is appropriate in cases where samples are expected to have similar total mRNA amounts, such replicate samples within the same condition. But when working with multiple experimental conditions, such as tissues and diagnostic groups, that are expected to markedly vary in their transcriptomes, adjusting for RNA composition differences is required to discover genes with true expression changes and discard the similarly-expressed ones.

The key take-home message for you is that if DGE analysis results in globally-shifted gene logFCs, these are likely driven by differences in sample RNA composition and have to be adjusted. TMM can be used to effectively scale library sizes under the assumption that most genes are non-DEGs. But as a note, TMM is not designed nor optimal to deal with cases of large assymetric differential expression, where a high proportion of genes are DE in one direction, and thus end up having similar read proportions after sampling across conditions.
...
Proper normalization is necessary to draw correct conclusions from RNA-seq data
